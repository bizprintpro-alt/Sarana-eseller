...
export default function ConfirmDelivery() {
  ...
  return (
    <View style={s.container}>
      ...
      <View style={s.tabs}>
        {([
          { key: 'pin' as const, label: '🔢 PIN код' },
          { key: 'photo' as const, label: '📷 Зураг' },
          { key: 'qr' as const, label: '📱 QR скан' },
        ]).map((t) => (
          <TouchableOpacity
            key={t.key}
      ...
      {method === 'pin' && (
        <View style={s.content}>
          ...
          />
          <TouchableOpacity
            style={[s.confirmBtn, !pin && { opacity: 0.5 }]}
            onPress={() => { if (pin.length === 4) router.back(); }}
      ...
      {method === 'photo' && (
        <View style={s.content}>
          <View style={s.cameraPlaceholder}>
            ...
            <Text style={s.cameraText}>Камер нээх</Text>
          </View>
          <TouchableOpacity style={s.confirmBtn}>
            <Text style={s.confirmText}>Зураг авч баталгаажуулах</Text>
          </TouchableOpacity>
        </View>
      )}
      ...
      {method === 'qr' && (
        <View style={s.content}>
          <View style={s.cameraPlaceholder}>
            <Text style={{ fontSize: 48 }}>📱</Text>
            <Text style={s.cameraText}>QR код скан хийх</Text>
          </View>
        </View>
      )}
    </View>
