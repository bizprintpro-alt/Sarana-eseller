...
export default function DomainSettingsPage() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center py-20 text-sm text-gray-400">Ачааллаж байна...</div>;
  }

  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">Домайн тохиргоо</h1>
        <p className="text-sm text-gray-500 mt-1">Өөрийн домайн нэрийг дэлгүүртэй холбох (жишээ: minii-salon.mn)</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-gray-900">Custom домайн</h3>
            <p className="text-xs text-gray-500">Жишээ: minii-salon.mn, myshop.com</p>
          </div>
        </div>

        <div className="flex gap-2">
          <input type="text" value={domain} onChange={(e) => setDomain(e.target.value)} placeholder="yourdomain.mn"
            className="flex-1 px-4 py-2.5 border border-gray-200 rounded-lg text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition" />
      ...
      {current && !current.verified && (
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
          <h3 className="text-sm font-bold text-gray-900">DNS тохируулах заавар</h3>
          <p className="text-sm text-gray-500">Домайн нэрийнхээ DNS тохиргоонд дараах бичлэгийг нэмнэ үү:</p>

          {/* DNS record table */}
          <div className="bg-gray-50 rounded-lg border border-gray-200 overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-gray-500 border-b border-gray-200 bg-gray-100/50">
              ...
              <tbody>
                <tr>
                  ...
                  <td className="px-4 py-3 font-mono text-gray-700">{current.domain}</td>
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => copyValue('cname.eseller.mn')}
                      className="w-7 h-7 rounded-lg bg-white border border-gray-200 flex items-center justify-center cursor-pointer hover:bg-gray-50 transition text-gray-400 hover:text-indigo-600">
          ...
          <button onClick={handleVerify} disabled={verifying}
            className={cn('flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-semibold border-none cursor-pointer transition',
              verifying ? 'bg-gray-200 text-gray-400' : 'bg-green-600 text-white hover:bg-green-700')}>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h3 className="text-sm font-bold text-gray-900">Домайн амжилттай холбогдсон</h3>
              <p className="text-xs text-gray-500">Таны хэрэглэгчид {current.domain} хаягаар хандаж болно</p>
            </div>
