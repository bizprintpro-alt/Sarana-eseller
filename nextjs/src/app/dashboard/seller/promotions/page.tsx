  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Урамшуулал</h1>
          <p className="text-gray-500 mt-1">Хямдралын код үүсгэх, удирдах</p>
        </div>
      ...
      {promos.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-700">Промо код байхгүй</h3>
          <p className="text-gray-400 mt-1">Шинэ хямдралын код үүсгээрэй</p>
        </div>
      ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  <th className="text-left p-4 text-xs font-semibold text-gray-500 uppercase">Код</th>
                  <th className="text-left p-4 text-xs font-semibold text-gray-500 uppercase">Хөнгөлөлт</th>
                  <th className="text-left p-4 text-xs font-semibold text-gray-500 uppercase">Мин. захиалга</th>
                  <th className="text-left p-4 text-xs font-semibold text-gray-500 uppercase">Дуусах огноо</th>
                  <th className="text-left p-4 text-xs font-semibold text-gray-500 uppercase">Хэрэглэсэн</th>
              ...
              <tbody>
                {promos.map((p) => (
                  <tr key={p.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="p-4 text-sm font-semibold text-gray-900">{p.percentage}%</td>
                    <td className="p-4 text-sm text-gray-700">{formatPrice(p.minOrder)}</td>
                    <td className="p-4 text-sm text-gray-500">{new Date(p.expiryDate).toLocaleDateString('mn-MN')}</td>
                    <td className="p-4 text-sm text-gray-700">{p.usedCount} удаа</td>
                    <td className="p-4">
                      <button
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl max-w-md w-full shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-gray-200 flex items-center justify-between">
              <h2 className="text-lg font-bold text-gray-900">Шинэ промо код</h2>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600 text-xl">&times;</button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Код</label>
                <input value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })} placeholder="SUMMER20" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm text-gray-900 font-mono uppercase focus:outline-none focus:ring-2 focus:ring-indigo-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Хөнгөлөлт (%)</label>
                <input type="number" value={form.percentage} onChange={(e) => setForm({ ...form, percentage: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Хамгийн бага захиалга</label>
                <input type="number" value={form.minOrder} onChange={(e) => setForm({ ...form, minOrder: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Дуусах огноо</label>
                <input type="date" value={form.expiryDate} onChange={(e) => setForm({ ...form, expiryDate: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
              </div>
