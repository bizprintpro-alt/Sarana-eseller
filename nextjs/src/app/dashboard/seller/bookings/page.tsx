...
const STATUS_MAP: Record<AppointmentStatus, { label: string; style: string }> = {
  ...
  no_show:     { label: 'Ирээгүй',       style: 'bg-gray-100 text-gray-600' },
};
...
export default function BookingsPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">Цаг захиалга</h1>
        <p className="text-sm text-gray-500 mt-0.5">Бүх цаг захиалгыг удирдах, баталгаажуулах</p>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Хэрэглэгч, утас, үйлчилгээ..."
              className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition" />
          </div>
          {/* Date range */}
          <div className="flex items-center gap-2">
            <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)}
              className="px-3 py-2 border border-gray-200 rounded-lg text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition bg-white cursor-pointer" />
          ...
          {FILTER_TABS.map((t) => (
            <button key={t.key} onClick={() => setFilterStatus(t.key)}
              className={cn('shrink-0 px-3 py-1.5 rounded-lg text-xs font-semibold border-none cursor-pointer transition',
                filterStatus === t.key ? 'bg-indigo-600 text-white' : 'bg-gray-50 text-gray-600 hover:bg-gray-100')}>
              {t.label}
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {filtered.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-sm font-semibold text-gray-400">Захиалга олдсонгүй</p>
          </div>
        ) : (
                  ...
                  return (
                    <tr key={a._id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-5 py-3.5">
                        <div className="font-semibold text-gray-900">{fmtDate(a.date)}</div>
                        <div className="text-xs text-gray-400 flex items-center gap-1 mt-0.5">
                          <Clock className="w-3 h-3" /> {a.startTime} – {a.endTime}
                        </div>
                      </td>
                      <td className="px-4 py-3.5">
                        <div className="font-semibold text-gray-900">{a.customerName}</div>
                        <div className="text-xs text-gray-400 flex items-center gap-1 mt-0.5">
                          <Phone className="w-3 h-3" /> {a.customerPhone}
                        </div>
                      </td>
                      <td className="px-4 py-3.5 text-gray-700">{a.serviceName}</td>
                      <td className="px-4 py-3.5 text-gray-600">{a.staffName || '—'}</td>
                      <td className="px-4 py-3.5">
                        <span className={cn('text-xs font-semibold px-2.5 py-1 rounded-full', st.style)}>
                          {st.label}
                        </span>
                      </td>
                      <td className="px-4 py-3.5 text-right font-bold text-gray-900">{formatPrice(a.total)}</td>
