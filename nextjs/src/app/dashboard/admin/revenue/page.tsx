    ...
    <div>
      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Өнөөдрийн орлого', value: formatMNT(data?.todayRevenue || 0), color: '#22C55E', icon: <DollarSign className="w-4 h-4" /> },
            { label: `${period} хоногийн орлого`, value: formatMNT(data?.total || 0), color: '#3B82F6', icon: <BarChart3 className="w-4 h-4" /> },
            { label: 'MRR (тооцоо)', value: formatMNT(Math.round((data?.total || 0) / period * 30)), color: '#D97706', icon: <TrendingUp className="w-4 h-4" /> },
            { label: 'Эх үүсвэрийн тоо', value: sources.length.toString(), color: '#8B5CF6', icon: <BarChart3 className="w-4 h-4" /> },
          ].map(m => (
