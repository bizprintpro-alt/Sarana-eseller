...
const COMMISSION_KEYS = [
  'commission_rate',           // platform default %
  'affiliate_rate',            // affiliate %
  'seller_free_commission',    // Free plan seller %
  'seller_standard_commission',
  'seller_ultimate_commission',
  'seller_ai_pro_commission',
];

// GET /api/admin/commission — get all commission config
export async function GET(req: NextRequest) {
  const admin = requireAdmin(req);
  if (admin instanceof NextResponse) return admin;

  const configs = await prisma.platformConfig.findMany({
    where: { key: { in: COMMISSION_KEYS } },
  });
...
// PATCH /api/admin/commission — update commission rates
export async function PATCH(req: NextRequest) {
  ...
  for (const u of updates) {
    await prisma.platformConfig.upsert({
      ...
      create: { key: u.key, value: u.value, updatedAt: new Date() },
    });
  }

  await prisma.adminLog.create({
    data: { adminId: admin.id, action: 'commission.update', after: body },
  });
