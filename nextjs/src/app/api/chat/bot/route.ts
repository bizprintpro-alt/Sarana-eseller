...
export async function POST(req: NextRequest) {
  try {
    // Anonymous bot endpoint — rate-limit by IP to stop Anthropic credit drain
    const ip = req.headers.get('x-forwarded-for')?.split(',')[0] || req.headers.get('x-real-ip') || 'unknown';
    const bucket = checkBucket(`chat:bot:${ip}`, 30, 60_000);
    if (bucket.limited) {
      return NextResponse.json(
        { reply: 'Хэт олон хүсэлт. Түр хүлээнэ үү.', needsHuman: false },
    ...
    if (typeof message !== 'string' || !message || message.length > 2000) {
      return NextResponse.json({ reply: 'Мессеж буруу байна', needsHuman: false }, { status: 400 });
    }
